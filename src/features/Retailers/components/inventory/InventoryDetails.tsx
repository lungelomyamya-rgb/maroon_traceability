...
export function InventoryDetails({
  ...
  const getExpiryStatus = (days: number) => {
    ...
    if (days <= 3) {
      return { color: 'text-red-600 bg-red-50', label: 'Critical - Expires soon' };
    }
    if (days <= 7) {
      return { color: 'text-yellow-600 bg-yellow-50', label: 'Warning - Expires soon' };
    }
    if (days <= 30) {
      return { color: 'text-blue-600 bg-blue-50', label: 'Good - Fresh' };
    }
