...
import { Button, Card, DocumentUpload } from '@/src/features/shared/ui';
// Import DocumentType from the document-upload component
...
export function DocumentVerification({ 
  ...
  const handleDocumentUpload = (docs: Document[]) => {
    ...
    const allVerified = documentTypes.every(docType => verifiedDocs[docType.value as unknown as string]);
  ...
  return (
    <Card className="p-6 space-y-4">
      ...
      <DocumentUpload 
        documentTypes={documentTypes.map(dt => dt.value)}
        onUpload={(files: File[]) => handleDocumentUpload(files.map(file => ({ file, status: 'uploading' as const, preview: URL.createObjectURL(file), type: 'id' })))}
        multiple
      />
