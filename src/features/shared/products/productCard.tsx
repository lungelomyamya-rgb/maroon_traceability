...
import { ProductTitle, ProductDetail, BlockchainDetail, Strong } from '@/src/features/shared/ui/typography';
...
export function ProductCard({ record }: ProductCardProps) {
  ...
  const handleVerify = () => {
    if (!rolePermissions?.canVerify) {
      ...
      const message = `${rolePermissions?.displayName || 'This role'} cannot verify products. Switch to Inspector or Retailer role.`;
      alert(message);
      return;
    }
