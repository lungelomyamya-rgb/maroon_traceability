...
export function InventoryList({ 
  ...
  const getExpiryStatus = (daysUntilExpiry: number) => {
    if (daysUntilExpiry < 0) return { color: 'text-red-600', label: 'Expired' };
    if (daysUntilExpiry <= 3) return { color: 'text-red-600', label: `${daysUntilExpiry} days` };
    if (daysUntilExpiry <= 7) return { color: 'text-yellow-600', label: `${daysUntilExpiry} days` };
