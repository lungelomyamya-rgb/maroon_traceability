...
export interface Web3Provider {
  ...
  sendTransaction?: (tx: { from: string; to: string; value: string; data: string }) => Promise<BlockchainTransaction>;
  eth?: {
    getChainId?: () => Promise<number>;
    getTransaction?: (txHash: string) => Promise<BlockchainTransaction>;
    getTransactionReceipt?: (txHash: string) => Promise<{ status: number }>;
