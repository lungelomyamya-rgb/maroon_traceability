// src/components/dashboard/LogisticsDashboard.tsx
'use client';

import { Truck, Package, MapPin, Clock, Navigation, Route } from 'lucide-react';
...
export function LogisticsDashboard() {
  ...
  const activeShipments = blockchainRecords.filter(p => p.status === 'In Transit').length;
  const deliveredToday = blockchainRecords.filter(p => p.status === 'Delivered').length;
  const pendingCollection = blockchainRecords.filter(p => !p.verified).length;
  const totalDeliveries = blockchainRecords.reduce((sum, p) => sum + p.verifications, 0);
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-green-600/90 to-green-700/70 rounded-2xl shadow-2xl max-w-4xl mx-auto w-full">
        ...
        <div className="relative z-10 text-center p-8">
          ...
          <PageTitle className="mb-2 text-white text-2xl sm:text-3xl">
            Welcome back, {typeof window !== 'undefined' ? (localStorage.getItem('userName') || 'Logistics Manager') : 'Logistics Manager'}!
          </PageTitle>
