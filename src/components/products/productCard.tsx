...
export function ProductCard({ record }: ProductCardProps) {
  ...
  const rolePermissions = currentUser?.role ? RolePermissionsService.getPermissions(currentUser.role) : null;

  const handleVerify = () => {
    if (!rolePermissions?.canVerify) {
      // Use a more user-friendly notification instead of alert
      const message = 'This role cannot verify products. Switch to Inspector or Retailer role.';
      alert(message);
      return;
    }
                  ...
                  <div className="w-24 h-24 bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center cursor-pointer hover:bg-gray-200 transition-colors"
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') {
                        setSelectedProduct(mapToProduct(record));
                      }
                    }}
                    role="button"
                    tabIndex={0}
